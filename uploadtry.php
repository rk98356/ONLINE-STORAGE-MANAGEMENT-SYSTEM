				    ...
				    foreach($ffs as $ff){
				        ...
				        if($type == 'Folders'){
					        if(is_dir($dir.'/'.$ff)){
								...
								$query = "SELECT * FROM users WHERE `folder_name`='uploads/".$_SESSION['user_id'].'/'.$ff."' AND `trashed` ='0'";
								//echo $query;
										    ...
										    <span style="float:right;">
					    						...
					    						<button class="btn  fa fa-ellipsis-v" type="button" data-toggle="dropdown"></button>
					 							<ul class="dropdown-menu">
					    						  ...
					    						  <li><a class="dropdown-item move_to_trash" data-path="'.$ff.'" data-type="folder" data-trash="1">Move To Trash</a></li>			    				
					    						</ul>
					 						</div></span>
										...
										echo '<div class="col-sm-4 box" style="font-family:Times New Roman, Times, serif;"><div class="inner_box"><span style="float:right;">
				    						...
				    						<button class="btn  fa fa-ellipsis-v" type="button" data-toggle="dropdown"></button>
				 							<ul class="dropdown-menu">
				    						  ...
				    						  <li><a  class="dropdown-item move_to_trash" data-path="'.$dir.'/'.$ff.'" data-trash="1">Move To Trash</a></li>			    						  
				    						</ul>
...
<?php
	if(isset($_POST['upload']))
	{
		...
		if (move_uploaded_file($_FILES['image']['tmp_name'],$target)) 
		{
			...
			$sizeofFile = $_FILES['image']['size'];
			//$image =  $_FILES ['image']['name'];

			$sql = "INSERT INTO `users`(`image`,`folder_name`,`created_by`,`creation_date`,`trashed`,`size`) VALUES ('".$target."','".$path_file."','".$_SESSION['username']."','".date("Y-m-d H:i:s")."','0','".$sizeofFile."')";
			//echo $sql;	
			mysqli_query($db,$sql);	
		...
		echo "<script>alert('No Space Left');</script>";
	}
	...
	if (isset($_POST['upload1']))
	{
		...
		if($_POST['foldername'] != "")
		{
			...
			$path_folder='uploads/'.$_SESSION['user_id'].'/'.$foldername;
			$db = mysqli_connect("localhost","root","","task_management");
			if(is_dir('uploads/'.$_SESSION['user_id']))
			{
				mkdir('uploads/'.$_SESSION['user_id'].'/'.$foldername);
			...
			foreach ($_FILES['files']['name'] as $i=>$name)
			{
				...
				{
					$target ='uploads/'.$_SESSION['user_id'].'/'.$foldername.'/'.$name;
